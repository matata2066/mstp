...
| 映射表查找失败 | 交易挂起，标记为"要素缺失"状态，通知运维补录 |
| 下游 API 调用超时 | 重试（指数退避，最多3次），超过重试次数标记为"发送失败" |
| 下游 API 返回失败 | 记录失败原因，交易标记为"支付失败" |
| 支付状态轮询超时 | 设置最大轮询次数/时间，超时标记为"状态未知"，需人工介入 |
| 动账确认不一致 | 标记为"待核实"，触发告警，需人工对账 |
