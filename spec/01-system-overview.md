...
| 维度 | 选型方向 | 说明 |
|------|----------|------|
| 后端框架 | Spring Boot 3.x (JDK 17) | 企业级 Java 框架，生态成熟 |
| 构建工具 | Maven (多模块) | 银行项目主流，CI/CD 集成方便 |
| 消息中间件 | Solace JMS/API | 与上游系统对接 |
| 数据库 | Oracle 19c+ | 满足银行合规与性能要求 |
| 前端框架 | Vue3 + Ant Design Vue | 企业级中后台 UI 方案 |
| 安全框架 | Spring Security + SPI | 保留 SSO/RBAC 外部接口 |
| 日志框架 | SLF4J + Logback | 分级日志，结构化输出 |
| API 文档 | OpenAPI 3.0 (Swagger) | 接口文档自动生成 |
| 交付方式 | Spring Boot Fat JAR | 单 JAR 部署，Apache 反向代理 + 证书校验 |

### 2.3 部署架构

```
                    ┌─────────────┐
                    │ Apache HTTPD │ ← TLS 证书校验
                    │ (Reverse     │
                    │  Proxy)      │
                    └──────┬──────┘
                           │
...
- 每个节点以 `java -jar mstp-app.jar` 启动，JDK 路径 `/opt/jdk/latest`
- Apache HTTPD 作为反向代理，负责 TLS 终结与客户端证书校验
